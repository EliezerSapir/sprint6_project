...
st.subheader('Older cars and newer cars sell for the highest price. Car models from 1974-2013 sell on average for less than 15K dollars.')

year = px.histogram(df, df.model_year, df.price, histfunc='avg')

st.bar_chart(year)

st.subheader('The graph below supports common sense, the new cars sell for the most while the fair and salvage categories consistently sell for the least.')

condition = px.histogram(df, df.condition, df.price, histfunc='avg')

st.bar_chart(condition)

st.subheader('The graph below takes this observation deeper, plotting price against model year, with colors marking the condition. We can see from this graph that the majority of cars in new condition were produced in the last 20 years. We can also easily see the outlier with the highest sale was from a car in good condition from 1999.')
...
choice7 = st.checkbox('2020')
if choice7:
    st.write(df[df['model_year'] == 2020]['price'].mean())
